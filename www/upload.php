...
if($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['inputData'])) {
    ...
    if (!$hasError) {
        ...
        // Insert document-category relationships into 'document_category' table
        foreach ($_POST['categories'] as $category_id) {
            $sql = "INSERT INTO document_category (document_id, category_id) VALUES (?, ?)";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "ii", $new_document_id, $category_id);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
        }
          ...
          <form action="upload.php" method="post" enctype="multipart/form-data" class="form-signin">
            ...
            <div class="form-group">
                <select class="form-control mb-2" id="categoriesSelect" name="categories[]" required multiple>
                    ...
                    <option value="new" id="addCategory" onclick="customAlert.alert('Enter the name of the new category:','Add New Category')">
